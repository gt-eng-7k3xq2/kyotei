...
## GARON R07リアルタイム実践共有MVP(2026-09-14夜〜)

R07の全参入通知・ユーザーの実践参入/明示的見送り/未確認・公式結果/確定配当を分離して記録し、iPhoneから実践入力ができ、結果確定後に仮想成績と実践成績を比較(回避できた損失・逃した利益・ROI増減)できる仕組み。**2026-09-14夜実装、実ntfy送信・実サイト取得・実AI呼出・Tailscale接続を伴う実通信試験は未実施(fixtureテストのみ完了)。**

- **既存Bridge(garon_ai_bridge_v1、WSL2内・127.0.0.1限定・AI案件承認/本番デプロイ可否を扱う機微API)とは完全に別プロセス・別ポート・別保存領域**として分離実装した(CEO確認済み)。`garon_ai_bridge_v1/src/server.js`は無変更。
- `scripts/lib/race_ops_event_store.js`(新規) — `scripts/lib/realtime_event_store.js`(同日昼実装)のcanonicalStringify/sha256Hex/appendEventLine/readEventObjectsを再利用し、`logs/race_ops_events_YYYY-MM-DD.jsonl`(追記専用、既存`logs/realtime_screening_events_*.jsonl`とは別ファイル・無変更)へ7種のイベント(`notification_opened`/`user_entry_recorded`/`user_skip_recorded`/`result_confirmed`/`result_void`/`result_fetch_failed`/`result_correction`)を保存する。judgmentEventId(判定イベントのID)で両ファイルを結合する。結果訂正は旧イベントを書き換えず追記のみ。
- `scripts/lib/race_ops_result_fetch.js`(新規) — `scripts/backfill_official_results.js`に追加した`module.exports`(`buildUrl`/`fetchResultPage`/`parseChakuju`/`parsePayout`/`loadVenueCodeMap`、`require.main===module`ガード追加済み)を再利用し、daikibo_archiveへは一切書き込まずに1レース分の結果だけを取得する。締切から3時間経過してもなお着順が取れない場合のみ「不成立疑い」として報告する(自動では確定イベントを書かない、推測しない)。
- `scripts/race_ops_server.js`(新規) — Windowsネイティブ(WSL2非経由、`http`標準モジュールのみ)の常駐HTTPサーバ、ポート既定47871。アプリ側トークンを持たない設計で、**Tailscale Serve(雄大さんが別途設定するtailnet内限定公開)の到達性そのものを唯一のアクセス制御とする**(ntfy通知・成果物・ログに秘密値を一切含めない設計)。`GET /`(当日一覧)・`GET /race?id=...`(判定内容閲覧・開封記録)・`POST /race/action`(実践参入/見送り記録、raceId形式を厳密に検証しWindowsパスの混入を拒否)・`GET /summary`(直近の運用分析)のみを提供し、R07本体・本番HTML・Bridgeへの書き込み経路は一切持たない。
- `scripts/race_ops_analysis.js`(新規) — 全通知/実践参入/明示的見送り/未確認の4区分を分離集計し(仮想成績と実践成績も混ぜない)、`reports/race_ops_analysis_YYYY-MM-DD.md`/`.json`を生成する。実践成績は「記録された合計投資額を判定時の推奨買い目比率で按分した近似値」であることを明示する(内訳までは記録しないため)。
- `scripts/realtime_screening.js` — 通知直前、`.env`の`RACE_OPS_PUBLIC_BASE_URL`(雄大さんがTailscale Serve設定後に手動設定する、秘密値を含まないtailnet URL)が設定されている場合のみ、ntfy通知にClickヘッダー(`scripts/lib/notify.js`に追加、未指定時は既存動作と完全に同じ)を付けて該当レースページへ直接飛べるようにする。未設定の間は何もしない(2026-09-14夜時点では未設定、実質no-op)。
- `gtools_bridge_local.html`(PC専用file://ページ、iPhoneから到達不可) — 「📅 今日の運用分析プリセットを適用」ボタンを追加。inputPathsは自由入力欄を設けず、当日日付を埋め込んだ固定リスト(R07運用データ・R07/Gツールソース・Bridge新規ソース)のみを送信する。
- `scripts/setup_scheduled_tasks.ps1`にタスク定義(`GARON_RaceOpsServer`常駐・`GARON_RaceOpsNightlyAnalysis`23:05)を追加済みだが**未登録**(管理者権限セッションでの実行が必要、`GARON_FieldBackfillBatch`と同じ前例)。
- **未完了(雄大さんご自身の作業が必要)**: ①Tailscaleを本PC・iPhoneへインストールしログイン ②`tailscale serve`で`race_ops_server.js`(ポート47871)を公開しtailnet URLを`.env`の`RACE_OPS_PUBLIC_BASE_URL`へ設定 ③管理者権限セッションから`setup_scheduled_tasks.ps1`を再実行し上記2タスクを登録。これらが完了するまでiPhoneからの実践入力・通知タップは機能しない(コード自体はテスト済みで、この3点が揃い次第動作する設計)。
