...
|---|---|---|---|
| `GARON_FieldBackfillBatch` | 毎日7:00(2026-09-02追加、RealtimeScreeningの前) | `scripts/run_field_backfill_batch.js --hours=20`(`run_field_backfill_batch.cmd`経由) | 過去日付の`daikibo_archive_*.json`のうちboats未マージ・オッズ欠損が残っている日を、`backfill_missing_fields.js`(kyoteibiyori.comの過去日付ページから再収集)で1日ずつ処理する。従来は雄大さんが気づいた時に手動起動する設計だった(CEOの「稼働時間は段階的に伸ばす、20時間→様子見→22時間→24時間」という方針で、意図的に手動運用にしていた)。**2026-09-02、CEO指示によりタスク化**。固定間隔(4分、2026-08-30のブロック解除後テストで完走した唯一の実測安全値)で、稼働予算20時間に達するか、サイトブロック検知・子プロセス異常終了のいずれかが起きると即座にバッチ全体を停止する(ブロック中に次の日へ進まない設計、`collect_batch.js`とは異なる)。日の途中で予算切れになっても打ち切らず、次回起動時にその日の続きから自動再開する。二重起動防止はPID生死確認ロック(`logs/.field_backfill_batch.lock`)。**2026-09-02、登録完了**: 非管理者セッションでの初回登録は権限不足で失敗したため、雄大さんに管理者権限セッションから`setup_scheduled_tasks.ps1`を実行してもらった。1回目は`RestartCount 0`が原因でタスクXMLが不正になり(`Register-ScheduledTask`がHRESULT 0x80041319で失敗)、`Register-GaronTask`共有関数を使う全18タスクの登録がまとめて失敗する事故が発生(スクリプト側の不具合、実行後に発覚・即修正)。`RestartCount`を1(30分後に1回だけ再試行)に変更して再実行し、全18タスク登録成功・`GARON_FieldBackfillBatch`もState=Ready・トリガー毎日07:00で確認済み。 |
| `GARON_RealtimeScreening` | 毎日8:00(2026-08-16、モーニング開催の1R締切8:32等に対応するため10:00→8:00に変更) | `scripts/realtime_screening.js`(`run_realtime_screening.cmd`経由) | T-10到達レースの抽出・参入判定・ntfy通知。稼働時間帯(8時台〜21時台)を過ぎるとプロセス自身が日次サマリー通知を送って終了する(タスクスケジューラは起動と失敗時再起動〈2分間隔・最大999回〉のみ担当)。抽出/判定が連続5件失敗するとntfyで異常アラートを送る(ただしkyoteibiyori.comへのスケジュール取得自体が失敗するケースはこのアラート対象外。サイトブロック監視は`GARON_SiteBlockMonitor`が別途担当)。判定結果は`logs/race_judgments_YYYY-MM-DD.json`に構造化保存する(`GARON_DraftSkipReason`が読む)。**2026-08-17追加**: `GARON_SiteBlockMonitor`が書く`logs/.site_block_state.json`を毎ループ読み、ブロック中(かつ直近20分以内に確認済み=状態が新しい)と分かっている間はkyoteibiyori.comへの実リクエスト自体をスキップして待機する。状態が無い/壊れている/20分以上古い(監視タスク停止の疑い)場合は信用せず通常通り自分でポーリングを試みる(安全側に倒す設計)。ブロック解除検知後は次のループ(最大4分後)で自動的に通常監視を再開する。**2026-08-21追加**: T-10抽出データを`daikibo_archive_YYYY-MM-DD.json`(当日分)へも保存する「相乗り」を実装(`scripts/lib/archive-entry.js`、`collect_playwright.js`とエントリ組み立てロジックを共有)。参入判定用に取得済みのデータを再利用するだけで**追加のkyoteibiyori.comリクエストは発生しない**。resulted/chakuju/payoutは未確定のまま保存し、夜間の`GARON_NightlyBackfill`が後から埋める。自宅Wi-Fiのブロックが続いている間、`collect_playwright.js`による別途の過去データ収集を行わずに済ませる目的(雄大さんの希望、テザリング経由のアクセス機会を極力RealtimeScreeningの本来業務に絞る)。ただしT-10に到達したレースしか捕捉できない(締切前中止等は取れない)ため、collect_playwright.jsに比べて抜けが出やすい。抜けの確認は`GARON_DataQualityScan`に委ねる。**2026-09-14追加(CEO指示「判定証拠の即時永続化」)**: `scripts/lib/realtime_event_store.js`(新規)により、評価した全レース(参入・見送り問わず)の判定内容(ルート・pEscape・買い目・1点ごとの金額・合計投資額等)を、判定確定直後に`logs/realtime_screening_events_YYYY-MM-DD.jsonl`(1行1イベントの追記専用JSONL、書き込みごとにfsync)へ保存するようにした。既存の`race_judgments_YYYY-MM-DD.json`(1日1回まとめ書き、買い目自体は含まない)・`.notified_races_YYYY-MM-DD.json`(二重送信防止専用)はいずれも変更・置き換えしていない(併存)。eventIdは判定結果(レースID・入力ハッシュ・買い目等)から決定論的に導出し現在時刻を含まないため、再起動後に同一判定を再評価しても重複保存されない。永続化に失敗した場合はそのレースの通知を見送り、ntfyで異常アラートを送る(証拠を失ったまま通知だけ進めない設計)。通知送信が成功した場合のみ、参照先の判定イベントIDを持つ`notification_sent`イベントを追記する。目的はレース結果確定後に仮想的中率・ROIを買い目単位で再計算できるようにすること(2026-09-14時点でこのJSONLを読む集計スクリプトはまだ無い、次の課題)。 |
...
## GARON R07リアルタイム実践共有MVP(2026-09-14夜実装、2026-09-15受入監査で恒久修正)

R07の全参入通知・ユーザーの実践参入/明示的見送り/未確認・公式結果/確定配当を分離して記録し、iPhoneから実践入力ができ、結果確定後に仮想成績と実践成績を比較(回避できた損失・逃した利益・ROI増減)できる仕組み。**2026-09-15時点で実ntfy送信・実サイト取得・実AI呼出・Tailscale接続を伴う実通信試験は未実施(fixtureテストのみ完了)。コードは有効化されていない(サーバー起動・タスク登録は未実施)。**

- **既存Bridge(garon_ai_bridge_v1、WSL2内・127.0.0.1限定・AI案件承認/本番デプロイ可否を扱う機微API)とは完全に別プロセス・別ポート・別保存領域**として分離実装(CEO確認済み)。`garon_ai_bridge_v1/src/server.js`は無変更。
- `scripts/lib/jst_time.js`(2026-09-15新規) — OSのタイムゾーン設定に依存しないJST日付/時刻計算(UTCエポック+9時間固定オフセット)。race_ops関連の全ファイルがこれを使う。TZ環境変数を変えた子プロセスで検証済み(`tests/race_ops_jst_time.regression.test.js`)。
- `scripts/lib/race_ops_event_store.js` — `logs/race_ops_events_YYYY-MM-DD.jsonl`(追記専用、既存`logs/realtime_screening_events_*.jsonl`とは別ファイル・無変更)へ7種のイベント(`notification_opened`/`user_entry_recorded`/`user_skip_recorded`/`result_confirmed`/`result_void`/`result_fetch_failed`/`result_correction`)を保存する。**2026-09-15修正**: `user_entry_recorded`は合計投資額の申告(旧`investmentAmount`)ではなく、R07推奨買い目1点ごとの実購入額(`actualBets`、`validateAndNormalizeActualBets()`で推奨買い目との過不足・重複・形式・100円単位・上限額を厳格検証)を必須にし、`totalInvestment`は常にサーバー側で買い目別金額から再計算する(申告値は信用しない)。
- `scripts/lib/race_ops_result_fetch.js` — `scripts/backfill_official_results.js`の`module.exports`(`require.main===module`ガード済み)を再利用し、daikibo_archiveへは一切書き込まずに1レース分の結果だけを取得する。**2026-09-15追加**: `fetchResultPage`に取得タイムアウト(既定20秒、race_ops経由は15秒)を追加(`AbortController`)。日付/時刻計算は`jst_time.js`のJST固定ロジックに統一。
- `scripts/race_ops_server.js` — Windowsネイティブ(WSL2非経由)の常駐HTTPサーバ、ポート既定47871、アプリ側トークン無し(Tailscale Serveのtailnet到達性のみがアクセス制御)。**2026-09-15修正**: ①`GET /race`は一切書き込みを行わない(開封記録は新設の`POST /race/open`という明示操作でのみ行う。通知プレビュー・Safariプリフェッチでの誤記録を防止) ②`GET /race`・`POST /race/action`・`POST /race/open`は`raceId`+`judgmentEventId`の組を必須にし、judgmentEventIdの実在・raceIdとの一致・対応する`notification_sent`の実在をすべて検証する ③POST系は`Content-Type: application/json`必須(CORSのpreflight機構によりブラウザ経由のクロスオリジンPOSTを構造的に阻止するCSRF対策)+Origin検証を追加 ④実践参入はR07推奨買い目を編集可能な形で初期表示し、`actualBets`配列で送信する ⑤起動時にPIDロック(`logs/.race_ops_server.lock`)で多重起動を防止。
- `scripts/race_ops_result_updater.js`(2026-09-15新設) — 通知済みレースの締切後、公式結果を安全な間隔で自動取得し続ける常駐プロセス(23:05の夜間集計だけでは「随時反映」の要件を満たさないため新設)。締切からの最低待機時間(既定8分)・取得失敗後のバックオフ(既定10分)・確定済み結果の再チェック間隔(既定30分、かつ確定から6時間以内のみ)により、同時取得数=1件・サイト負荷抑制を両立する。`scripts/lib/race_ops_result_fetch.js`のタイムアウトをそのまま使う。PIDロック(`logs/.race_ops_result_updater.lock`)で多重起動を防止。
- `scripts/race_ops_analysis.js` — **2026-09-15修正**: ①母集団を「`entered:true`」だけでなく「対応する`notification_sent`が実在する判定」に限定し、`raceId`単位ではなく`judgmentEventId`単位で扱う(同一レースへの再判定・再通知があっても取り違えない) ②実践成績は`computeActualNetFromBets()`で買い目別実購入額から実額計算する(旧`computeApproxActualNet()`は投資額フィールドが欠落しROIが常にnullになるバグがあったため廃止) ③的中買い目があるのに払戻金額の解析に失敗した場合は0円(不的中)扱いにせず「未解決」として集計から除外し件数のみ報告する ④結果訂正: 確定から6時間以内に限り再取得を許し、内容が変われば旧イベントを保持したまま`result_correction`を追記する(`fetchAndRecordResult()`)。
- `scripts/realtime_screening.js` — 通知直前、`.env`の`RACE_OPS_PUBLIC_BASE_URL`が設定されている場合のみntfy通知にClickヘッダーを付与。**2026-09-15修正**: URLに`raceId`だけでなく`judgmentEventId`も含める(表示・操作対象を一意に固定するため)。未設定の間は何もしない(2026-09-15時点では未設定、実質no-op)。
- `gtools_bridge_local.html`(PC専用file://ページ、iPhoneから到達不可) — 「📅 今日の運用分析プリセットを適用」ボタン。inputPathsは固定リストのみ(自由入力不可)。
- **タスク登録の分離(2026-09-15、受入監査指摘)**: `scripts/setup_scheduled_tasks.ps1`(既存18〜20タスクを丸ごと再登録する共有スクリプト)からRaceOpsタスクの定義を完全に除去し、`scripts/register_race_ops_tasks.ps1`/`scripts/unregister_race_ops_tasks.ps1`/`scripts/status_race_ops_tasks.ps1`という専用スクリプト群へ分離した(`GARON_RaceOpsServer`・`GARON_RaceOpsResultUpdater`・`GARON_RaceOpsNightlyAnalysis`の3タスクのみを対象にし、既存タスクには一切触れない。`tests/race_ops_task_isolation.regression.test.js`で静的に検証済み)。実行対応の`.cmd`: `run_race_ops_server.cmd`/`run_race_ops_result_updater.cmd`/`run_race_ops_analysis.cmd`。**いずれも未登録**(管理者権限セッションでの実行が必要、`GARON_FieldBackfillBatch`と同じ前例)。
- **未完了(雄大さんご自身の作業が必要)**: ①Tailscaleを本PC・iPhoneへインストールしログイン ②`tailscale serve`で`race_ops_server.js`(ポート47871)を公開しtailnet URLを`.env`の`RACE_OPS_PUBLIC_BASE_URL`へ設定 ③管理者権限セッションから`scripts/register_race_ops_tasks.ps1`を実行し上記3タスクを登録。これらが完了するまでiPhoneからの実践入力・通知タップ・結果の自動反映は機能しない(コード自体はテスト済みで、この3点が揃い次第動作する設計)。
- **2026-09-15、2回目の受入監査で恒久修正した2点**:
  1. 「リアルタイム結果が画面に反映されていない」— `scripts/race_ops_server.js`のGET `/race`は、その場で`resolveCurrentResult`/`computeVirtualNet`/`computeActualNetFromBets`(いずれも`race_ops_analysis.js`、読み取り専用)を呼び、公式着順・配当・仮想成績(的中/不的中・投資・払戻・純損益)・実践参入済みなら実践成績も表示するようにした。結果未確定時は損失扱いせず「結果未確定」と表示する(推測しない)。一覧画面にも結果状態バッジを追加。GET `/summary`は23:05生成の静的JSONを読むのをやめ、`computeLiveAnalysis()`(`race_ops_analysis.js`から新規切り出し、main()の夜間集計ロジックと共有)でその場で都度集計するよう変更した(読み取り専用、ネットワーク取得なし)。GETによる書き込みが無いことは変更後も維持・再確認済み。
  2. 「0円参入を拒否する」— `scripts/lib/race_ops_event_store.js`の`validateAndNormalizeActualBets()`に、買い目別は0円(未購入)を許容しつつ合計が100円未満なら拒否する検証を追加(エラーメッセージ「購入額を入力するか、見送りを選択してください」)。`recordUserEntry()`・`POST /race/action`の両レイヤーで検証される。
  - 追加テスト: `tests/race_ops_event_store.regression.test.js`(+6項目)・`tests/race_ops_server.regression.test.js`(+17項目、テスト22a〜22e)。全274項目PASS(既存の合格分は変更・後退なし)。
...
7. **学習成果物(モデル再学習・候補モデル比較)を外部(Codex等)から受け取る、または内部で生成する際は、必ず以下を同梱・記録すること(2026-09-05、GARON-20260905-001の反証部隊指摘を受けて追加)。** kinsetsu6m候補(P1)の独立監査で、学習に実際使われた生データの出所が成果物からもGARON側の記録からも追跡不能という調査ギャップが発覚した(結論への実害はなかったが、独立した安全網が偶然働いた結果に過ぎない)。今後はこれを偶然に頼らず構造的に担保する。
   - 生学習データそのもの、または内容ハッシュ付きの不変スナップショット
   - 生データのSHA-256
   - 特徴量生成コード(学習・推論の両方で同一のものを使うこと)
   - 生成後の特徴量(モデルへの入力ベクトル)のハッシュ
   - 学習対象レースID一覧(日付+会場+レース番号等で一意に特定できる形)
   - 学習コード本体
   - 実行環境(Python・NumPy・scikit-learn・SciPy等、モデルの再現に影響し得る主要ライブラリ)のバージョン
   - 乱数シード・ハイパーパラメータ一式
   - 学習済みモデル本体
   - 展開後(gzip解凍後)モデルのハッシュ(**圧縮ファイル自体のハッシュではなく、これを再現性確認の基準にすること**。HistGradientBoostingClassifier等はハードウェア〈CPU/BLAS〉依存でモデルのシリアライズが環境ごとに異なり得るため、圧縮バイト列の一致を再現性の基準にすると、正当な候補を誤って「未検証」と判定しかねない。予測値〈`predict_proba`等〉が既知の参照値と一致するかで真正性を判断すること)
   - 固定評価データ(学習に使っていない、評価専用のレース集合)
   - 再現コマンド(誰が実行しても同じ検証ができる具体的な手順)

   **追加(2026-09-05、GARON-20260905-009の反証部隊指摘を受けて追加)**: `daikibo_archive_*.json`はバックフィルにより後日中身が書き換わるため、「パスとハッシュだけを成果物として記録する」方式は、原本が変化した後の再検証で数値がズレる(結論への影響は無くても再現性の見た目を損なう)ことが判明した。今後は以下も遵守すること。
   - **研究開始時に実際に読み込んだ入力データを物理的に複製保存すること。** 元の`daikibo_archive_*.json`等へのパスだけを成果物としない(パス参照は原本の変化に追随してしまうため、再現性の担保にならない)
   - スナップショットは研究専用の新規ファイルとして保存する(既存の凍結データ・他案件のスナップショットを上書きしない)
   - raceIdHash(レース単位の識別子一覧のハッシュ)だけでなく、**スナップショットファイル全体のSHA-256**も保存する
   - スナップショットにmanifest情報(作成日時JST、対象期間、件数、生成コード、生成コマンド)を同梱する
   - 学習・評価・最終比較のデータは別ファイルへ分離する(1つのファイルに全工程を混在させない)
   - **一度保存したスナップショットは上書き禁止。** 再実行時は原本(daikibo_archive等)ではなく保存済みスナップショットを読むこと
   - 原本データ(`daikibo_archive_*.json`等)自体の変更は禁止(スナップショットはあくまでコピー)
   - スナップショットに個人情報・認証情報・APIキー・不要なログを含めないこと
